...
| Borde del mundo | `WORLD_BORDER_RADIUS` en el `.env`, en bloques a cada lado del spawn, igual en las tres dimensiones. **10.000 desde el 2026-09-08** (venía de 6.000, y antes de 4.000). Se aplica con `servidor/configurar-borde.py`, que se niega a achicarlo salvo con `--achicar`: lo que quede afuera del borde nuevo no se puede volver a habitar. |
